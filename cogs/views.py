...
class mySelect(View):
    ...
    @discord.ui.select(placeholder="Choose a category",
                       options=[
                          ...
                          discord.SelectOption(label="Moderation", value="3", description="Shows bot's moderation commands", emoji="⚙️"),
                          discord.SelectOption(label="Gacha", value="4", description="Shows bot's gacha commands", emoji="🎱"),
                          discord.SelectOption(label="Account & Premium", value="5", description="Shows account and premium commands", emoji="💎"),
                          discord.SelectOption(label="NSFW", value="6", description="Shows bot's nsfw commands", emoji="🔞"),
                          discord.SelectOption(label="Security", value="12", description="Shows security commands", emoji="🛡️"),
                          discord.SelectOption(label="Ticket & Verification", value="7", description="Shows bot's ticket and verify commands", emoji="📠"),
                          discord.SelectOption(label="Giveaway", value="8", description="Shows bot's giveaway commands", emoji="🎉"),
                          discord.SelectOption(label="Activity", value="13", description="Shows activity commands", emoji="👤"),
                          discord.SelectOption(label="Messages", value="11", description="Shows message counter commands", emoji="💬"),
                          discord.SelectOption(label="Reaction Roles", value="14", description="Shows reaction role commands", emoji="🎭"),
    ...
    async def select_callback(self, interaction, select):
        if interaction.user.id == self.authorID:
            if select.values[0] == "1":
                em = discord.Embed(title="Equinox Navigator - Utilities", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)", color=0xffffff)
                em.set_author(name="Utilities Commands List", icon_url=state.client.user.avatar)
                em.add_field(name="> </ping>", value="```Shows bot's latency.```", inline=False)
                em.add_field(name="> </invite>", value="```Shows bot's invite.```", inline=False)
                em.add_field(name="> </help>", value="```Display this message.```", inline=False)
                em.add_field(name="> </qrcode> `<parm>`", value="```Make a qr-code from link, text, or emoji.```", inline=False)
                em.add_field(name="> </daysbetween> `<mm/yy>` `<mm/yy>`", value="```Calculate the day(s) between two dates (dd/mm/yy).```", inline=False)
                em.add_field(name="> </poll> `first_emoji` `first_option` `second_emoji` `second_option` `third_emoji` `third_option`", value="```Make a poll from 3 emojis and 3 options. (Manage messages required)```", inline=False)
                em.add_field(name="> </def> `<word>`", value="```Shows word's type, definition, and example if there is any.```", inline=False)
                em.add_field(name="> </translate> `<arg>` `<lang>`", value="```Translate a text to another language (25 languages available).```", inline=False)
                em.add_field(name="> </fandom> `<search>` `<category>`", value="```Shows summary of a fandom page of a search from a category.```", inline=False)
                em.add_field(name="> </timedif> `<id1>` `<id2>`", value="```Shows the differences between two relevant ids (message, guild, channels, etc).```", inline=False)
                em.add_field(name="> </wiki> `<query>`", value="```Find information on wikipedia.```", inline=False)
                em.add_field(name="> </steal> `<emoji>` `<name>`", value="```Steal an emoji from another server.```", inline=False)
                em.add_field(name="> </graph> `<function>`", value="```Create a graph with a function.```", inline=False)
                em.add_field(name="> </embed> `<title>` `[description]` `[color]` `[image]` `[thumbnail]` `[footer]`", value="```Build an embed with a visual editor.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "3":
                em = discord.Embed(title="Equinox Navigator - Moderation", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /channel or /user", color=0xffffff)
                em.set_author(name="Moderation Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </channel lockdown>", value="```Lockdown a channel (manage channels required).```", inline=False)
                em.add_field(name="> </channel unlockdown>", value="```Unlock a channel (manage channels required).```", inline=False)
                em.add_field(name="> </channel private>", value="```Private a channel (manage channels required).```", inline=False)
                em.add_field(name="> </channel unprivate>", value="```Unprivate a channel (manage channels required).```", inline=False)
                em.add_field(name="> </channel slowmode> `<slowmode>`", value="```Set slowmode in current channel in seconds (manage channels required).```", inline=False)
                em.add_field(name="> </channel clone> `[channel]`", value="```Clone a channel (manage channels required).```", inline=False)
                em.add_field(name="> </user kick> `<member>`", value="```Kick a member from the server.```", inline=False)
                em.add_field(name="> </user ban> `<member>`", value="```Ban a member from the server.```", inline=False)
                em.add_field(name="> </user unban> `<memberid>`", value="```Unban a user by ID.```", inline=False)
                em.add_field(name="> </purge> `<bot_messages>` `<embed_messages>` `<user_messages>`", value="```Purge messages in a channel (manage messages required).```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "4":
                em = discord.Embed(title="Equinox Navigator - Gacha", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /gacha", color=0xffffff)
                em.set_author(name="Gacha Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </gacha roll> `item`", value="```Roll to get a title, boost luck with items.```", inline=False)
                em.add_field(name="> </gacha craft> `<item>` `amount`", value="```Craft items using ingredients```", inline=False)
                em.add_field(name="> </gacha inventory> `[member]` `type`", value="```Show a member's inventory.```", inline=False)
                em.add_field(name="> </gacha shop>", value="```Show the gacha shop.```", inline=False)
                em.add_field(name="> </gacha daily>", value="```Open a daily chest of 3 random potions (premium).```", inline=False)
                em.add_field(name="> </gacha flex> `[member]`", value="```Shows a user's gacha stats.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "5":
                ...
                em = discord.Embed(title="Equinox Navigator - Account & Premium", description=f"Your premium status: **{tier}**\n<> = Required Arguement(s)\n[] = Optional Arguement(s)\nPrefix with /account", color=0xffffff)
                ...
                em.add_field(name="> Little time rolling", value="```Roll time are reduced to 2-3s!```", inline=False)
                ...
                em.add_field(name="> Unlimited ticket system", value="```Elite users can deploy unlimited ticket systems into their server!```", inline=False)
                em.add_field(name="> Customizable ticket system", value="```With premium, you can fully personalize your ticket system!```", inline=False)
                ...
                em.add_field(name="> </account info>", value="```View your account status.```", inline=False)
                em.add_field(name="> </account login>", value="```Login to your database using email.```", inline=False)
                em.add_field(name="> </account unlink>", value="```Unlink your email from the bot.```", inline=False)
                em.add_field(name="> </account redeem>", value="```Redeem a premium code.```", inline=False)
                em.add_field(name="> </account reset>", value="```Reset your databases (24h cooldown).```", inline=False)
                em.add_field(name="> </premium> `[member]`", value="```Check your premium status.```", inline=False)
            ...
            if select.values[0] == "7":
                em = discord.Embed(title="Equinox Navigator - Ticket & Verification", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /ticket", color=0xffffff)
                em.set_author(name="Ticket & Verification Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </ticket create> `<category>` `[title]` `[description]` `[image]` `[role]` `[ticket_message]`", value="```Make a ticket system.```", inline=False)
                em.add_field(name="> </ticket add> `<member>`", value="```Add a member to a ticket.```", inline=False)
                em.add_field(name="> </ticket remove> `<member>`", value="```Remove a member from a ticket.```", inline=False)
                em.add_field(name="> </make_verify> `[rev_role]` `[add_role]` `[message]` `[image]`", value="```Make a verification system.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "8":
                em = discord.Embed(title="Equinox Navigator - Giveaway", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /giveaway", color=0xffffff)
                em.set_author(name="Giveaway Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </giveaway create> `<duration>` `<prize>` `[hosts]`", value="```Start a giveaway, duration must be less than 7 days.```", inline=False)
                em.add_field(name="> </giveaway manage> `<message_id>` `<action>`", value="```Reroll or finalize a giveaway.```", inline=False)
                em.add_field(name="> </giveaway console>", value="```Show all giveaway(s) within the server.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "6":
                if interaction.channel.is_nsfw():
                    ...
                    em.add_field(name="> </nsfw> `[category]`", value="```Shows NSFW content (normal users).```", inline=False)
                    em.add_field(name="> </prem_nsfw> `[category]`", value="```Shows unrestricted premium NSFW content.```", inline=False)
                    em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                    await interaction.response.edit_message(embed=em, view=self)
                else:
                    await interaction.response.defer()
            if select.values[0] == "11":
                em = discord.Embed(title="Equinox Navigator - Messages", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /messages", color=0xffffff)
                em.set_author(name="Message Counter Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </messages count> `[user]`", value="```Shows a user's message counts.```", inline=False)
                em.add_field(name="> </messages toggle> `<option>`", value="```Enable/disable the server's message counter system.```", inline=False)
                em.add_field(name="> </messages leaderboard>", value="```Shows top users with the most messages.```", inline=False)
                em.add_field(name="> </messages deduct> `<user>` `<amount>`", value="```Remove messages from a user.```", inline=False)
                em.add_field(name="> </messages give> `<to_user>` `<amount>`", value="```Transfer messages to another user.```", inline=False)
                em.add_field(name="> </messages blacklist> `<action>` `<channel>`", value="```Blacklist/unblacklist a channel from the message counter.```", inline=False)
                em.add_field(name="> </messages blacklist_view>", value="```View blacklisted channels.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "12":
                em = discord.Embed(title="Equinox Navigator - Security", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\nPrefix with /scam, /auditlog, or /codehelper", color=0xffffff)
                em.set_author(name="Security Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </scam enable>", value="```Enable Anti-Scam in this channel.```", inline=False)
                em.add_field(name="> </scam disable>", value="```Disable Anti-Scam in this channel.```", inline=False)
                em.add_field(name="> </scam log> `<channel>`", value="```Set a channel for scam logs.```", inline=False)
                em.add_field(name="> </scam test> `<text>`", value="```Test the scam scanner against custom text.```", inline=False)
                em.add_field(name="> </scam stats>", value="```View scam pattern confidence & feedback stats.```", inline=False)
                em.add_field(name="> </scam debug>", value="```Diagnose readiness & config.```", inline=False)
                em.add_field(name="> </scam phrase> `[phrase]` `[action]`", value="```Manage whitelisted phrases.```", inline=False)
                em.add_field(name="> </scam whitelist> `[user]` `[role]` `[action]`", value="```Manage scam shield whitelist.```", inline=False)
                em.add_field(name="> </scam status>", value="```Show current security configuration.```", inline=False)
                em.add_field(name="> </auditlog toggle> `<state>`", value="```Enable or disable audit logging.```", inline=False)
                em.add_field(name="> </auditlog set> `<channel>`", value="```Set a channel for audit log entries.```", inline=False)
                em.add_field(name="> </auditlog view> `<duration>`", value="```Download server audit logs.```", inline=False)
                em.add_field(name="> </codehelper enable>", value="```Enable Inline Code Helper in this channel.```", inline=False)
                em.add_field(name="> </codehelper disable>", value="```Disable Inline Code Helper in this channel.```", inline=False)
                em.add_field(name="> </lint> `<code>`", value="```Check Python code for syntax errors.```", inline=False)
                em.add_field(name="> </youngest> `<duration>`", value="```Show youngest accounts in the server.```", inline=False)
                em.add_field(name="> </block_ad> `<state>`", value="```Block Discord invite links as spam.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "13":
                em = discord.Embed(title="Equinox Navigator - Activity", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /activity", color=0xffffff)
                em.set_author(name="Activity Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </activity status> `<status>`", value="```View members by presence status or stats.```", inline=False)
                em.add_field(name="> </activity games>", value="```Show the top games being played.```", inline=False)
                em.add_field(name="> </activity now> `<user>`", value="```See what a user is currently playing.```", inline=False)
                em.add_field(name="> </activity role> `<role>`", value="```View presence stats for a role.```", inline=False)
                em.add_field(name="> </activity privacy>", value="```Manage your privacy and tracking settings.```", inline=False)
                em.add_field(name="> </activity members>", value="```Advanced member analytics.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
            if select.values[0] == "14":
                em = discord.Embed(title="Equinox Navigator - Reaction Roles", description="<> = Required Arguement(s)\n[] = Optional Arguement(s)\n Prefix with /reactionrole", color=0xffffff)
                em.set_author(name="Reaction Role Commands List", icon_url=interaction.user.avatar)
                em.add_field(name="> </reactionrole game>", value="```Make a selection role (Dev only).```", inline=False)
                em.add_field(name="> </reactionrole template> `<name>` `<action>`", value="```Add or remove a reaction role template.```", inline=False)
                em.add_field(name="> </reactionrole edit> `<name>` `<role>` `<action>` `[emoji]`", value="```Add or remove roles from a template.```", inline=False)
                em.add_field(name="> </reactionrole embed> `[name]` `[title]` `[color]`", value="```View or edit embed of a template.```", inline=False)
                em.add_field(name="> </reactionrole deploy> `<name>` `[channel]`", value="```Deploy a reaction role template to a channel.```", inline=False)
                em.set_footer(text="Equinoxxxx <3", icon_url=f'{interaction.user.avatar}')
                await interaction.response.edit_message(embed=em, view=self)
